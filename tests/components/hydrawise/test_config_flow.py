...
async def test_flow_import_success(
    hass: HomeAssistant, mock_legacy_pydrawise: AsyncMock, user: User
) -> None:
    """Test that we can import a YAML config."""
    mock_legacy_pydrawise.get_user.return_value = User
    result = await hass.config_entries.flow.async_init(
        DOMAIN,
...
async def test_flow_import_api_error(
    ...
    mock_legacy_pydrawise.get_user.side_effect = ClientError
    result = await hass.config_entries.flow.async_init(
        DOMAIN,
        context={"source": config_entries.SOURCE_IMPORT},
        data={
...
async def test_flow_import_connect_timeout(
    ...
    mock_legacy_pydrawise.get_user.side_effect = TimeoutError
    result = await hass.config_entries.flow.async_init(
        DOMAIN,
        context={"source": config_entries.SOURCE_IMPORT},
        data={
